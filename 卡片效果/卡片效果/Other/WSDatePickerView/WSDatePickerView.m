...
@interface WSDatePickerView ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    ...
    NSMutableArray *_monthCopyArray;
    ...
    NSMutableArray *_hourCopyArray;
    
    NSMutableArray *_minuteArray;
    
    NSMutableArray *_minuteCopyArray;
...
-(void)defaultConfig {
    ...
    _monthCopyArray = [self setArray:_monthCopyArray];
    _hourCopyArray = [self setArray:_hourCopyArray];
    _minuteCopyArray = [self setArray:_minuteCopyArray];
    
    _minYear = MINYEAR;
    _maxYear = MAXYEAR;
    
    self.isNoShowNoSelect = false;
    for (int i=0; i<60; i++) {
        NSString *num = [NSString stringWithFormat:@"%02d",i];
        if (0<i && i<=12){
            [_monthArray addObject:num];
            [_monthCopyArray addObject:num];
        }
        if (i<24){
            [_hourArray addObject:num];
            [_hourCopyArray addObject:num];
            
        }
        [_minuteArray addObject:num];
        [_minuteCopyArray addObject:num];
...
- (void)setIsNoShowNoSelect:(BOOL)isNoShowNoSelect{
    _isNoShowNoSelect = isNoShowNoSelect;
    [_yearArray removeAllObjects];
    NSInteger min = MINYEAR;
    NSInteger max = MAXYEAR;
    if (_isNoShowNoSelect) {
        if (min < [self.minLimitDate year] && self.minLimitDate != nil) {
            min = [self.minLimitDate year];
            _minYear = min;
        }
        if (max > [self.maxLimitDate year] && self.maxLimitDate != nil) {
            max = [self.maxLimitDate year];
            _maxYear = max;
        }
    }
...
-(NSArray *)getNumberOfRowsInComponent {
    ...
    if (_isNoShowNoSelect) {
        NSInteger yc = _maxYear - _minYear;
        if (yearIndex > yc) {
            yearIndex = yc;
        }
    }
    NSInteger monthNum = [self MonthsfromYear:[_yearArray[yearIndex] integerValue]];
...
- (NSInteger)DaysfromYear:(NSInteger)year andMonth:(NSInteger)month
{
    ...
    NSInteger allNum = num;
    if (self.isNoShowNoSelect == true) {
        if (year == self.minLimitDate.year && month == self.minLimitDate.month) {
            NSInteger num1 = num - self.minLimitDate.day + 1;
            
            num = num1;
            
        }else if (year == self.maxLimitDate.year && month == self.maxLimitDate.month) {
            NSInteger num1 = self.maxLimitDate.day;
            num = num1;
        }
        
    }
    
    [self setdayArray:num fromYear:year andMonth:month andAllnum:allNum];
    return num;
}

//设置每月的天数数组
- (void)setdayArray:(NSInteger)num fromYear:(NSInteger)year andMonth:(NSInteger)month andAllnum:(NSInteger)allnum
{
    [_dayArray removeAllObjects];
    if (self.isNoShowNoSelect == true) {
        if (year == self.minLimitDate.year && month == self.minLimitDate.month) {
            for (int i=(int)(allnum - num + 1); i<=allnum; i++) {
...
{
    NSInteger min_year  = self.minLimitDate.year;
    if (min_year == year) {
        NSInteger num = 12 - self.minLimitDate.month + 1;
        _monthArray = [[NSMutableArray alloc] initWithArray:[_monthCopyArray subarrayWithRange:NSMakeRange(self.minLimitDate.month - 1, num)]];
        return num;
    }
    
    NSInteger max_year = self.maxLimitDate.year;
    
    if (max_year == year) {
        NSInteger num = self.maxLimitDate.month;
        _monthArray = [[NSMutableArray alloc] initWithArray:[_monthCopyArray subarrayWithRange:NSMakeRange(0 , num)]];
        
        return num;
    }
    _monthArray = [[NSMutableArray alloc] initWithArray:_monthCopyArray];
...
- (void)getNowDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (self.isNoShowNoSelect == true) {
        _minYear = self.minLimitDate.year;
    }
    ...
    if (_isNoShowNoSelect) {
        if (_minYear == _minLimitDate.year) {
            monthIndex = date.month - _minLimitDate.month ;
            if (date.month == _minLimitDate.month) {
                dayIndex = date.day - _minLimitDate.day;
            }
        }
